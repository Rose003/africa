    ...
    useEffect(() => {
        const fetchProductDetails = async () => {
            try {
                ...
                const response = await axios.get(`https://thrift-backend-1974.onrender.com/products/product/${id}`);
                ...
                <Row>
                    <Col md={6}>
                        <Card className='product-image'>
                            <Card.Img variant="top" src={`https://thrift-backend-1974.onrender.com/${product.image}`} crossOrigin="anonymous" />
                        </Card>
                    </Col>
                    <Col md={6}>
                        <h1>{product.name}</h1>
